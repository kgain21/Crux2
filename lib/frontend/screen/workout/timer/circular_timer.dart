...
import 'package:crux/frontend/screen/workout/timer/timer_painter.dart';
...
class CircularTimer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TimerBloc, TimerState>(
      cubit: BlocProvider.of<TimerBloc>(context),
      builder: (context, state) {
        return GestureDetector(
          onTap: () {
            BlocProvider.of<TimerBloc>(context).add(TimerTapped(
              exerciseTitle: timerState.exerciseTitle,
              timer: timerState.timer,
              controllerStartValue: timerState.controllerStartValue,
            ));
            startStopTimer();
          },
          onLongPress: () {
            BlocProvider.of<TimerBloc>(context).add(TimerLongPressed(
              exerciseTitle: timerState.exerciseTitle,
              timer: timerState.timer,
              controllerStartValue: timerState.controllerStartValue,
            ));
...
//                  color: Colors.primaries[Random().nextInt(Colors.primaries.length)]
  ...
  /// forward and false is reverse. This method is used as the onTap callback
  /// of the [WorkoutTimer].
  ///
  ...
  /// Resets the timer based on its current status. This method is used as the
  /// onLongPress callback of the circular timer.
  void resetTimer() {
    if (timerController.isAnimating) {
      timerController.stop(canceled: false);
